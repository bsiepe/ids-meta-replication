...
```{r, eval = FALSE}
source("functions.R")
diff_comp <- compare_point_ests("models", "models/replication",
                                random_effects = TRUE)
...
saveRDS(df_diff_comp, here("output/replication/point_estimates_comparison.RDS"))
```

We reload the dataframe and show the results:
```{r}
df_diff_comp <- readRDS(here("output/replication/point_estimates_comparison.RDS"))
```



In addition, we can compare random effect standard deviations:
...
# combine to df
df_weights <- do.call(rbind, weights_list)

# remove irrelevant string
colnames(df_weights) <- gsub("AR_", "", colnames(df_weights))
  
df_weights <- df_weights |> 
  as.data.frame() |> 
  tibble::rownames_to_column(var = "weights") |> 
  mutate(weights = gsub("weights_", "", weights))

# only investigate loo weights here 
df_weights |> 
  filter(grepl("loo", weights)) |> 
  pivot_longer(cols = !weights, names_to = "model") |> 
  group_by(weights) |> 
...
# create file names
df_robust <- df_robust |> 
  mutate(filename = paste("~/ids-meta-replication/models/replication/", outcomes, "_", predictors, "_m_", "rep.rds", sep = ""))
...
F0_rep <- readRDS("~/ids-meta-replication/models/replication/F0_task_environment_language_age_report_rep.rds")
...
VSA_rep <- readRDS("~/ids-meta-replication/models/replication/VSA_environment_report_rep.rds")
